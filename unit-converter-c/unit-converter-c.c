...
int main() {
    ...
    if (category == 'T') {
        ...
        printf("Here is a list of conversations to choose from: \n");
        printf("Enter 1 for USD to Euro (european currency). \n");
        printf("Enter 2 for USD to JPY (japeanse currency). \n");
        printf("Enter 3 for USD to RMB (chinease currency). \n");
        ...
        if (currencyChoice == 1) {
            ...
            USDtoJPY = userinputUSDtoJPY * 115.68;
            printf("JPY: %.2f", USDtoJPY);
        }
        else if (currencyChoice == 3) {
            printf("Please enter the USD amount: \n");
            scanf("%d", &userinputUSDtoRMB);
            USDtoRMB = userinputUSDtoRMB * 6.32;
